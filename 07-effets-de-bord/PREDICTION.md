...
- nombre = 10 (modifié ou pas ?) pas mofifier
- tableau[0] = 101 (modifié ou pas ?) mofifier
```
...
Réponse : Le nombre (int) a été copié, donc la fonction n'a changé que sa copie. Le tableau (int[]) a été partagé (référence), donc la fonction a pu modifier l'original.
